...
/**
 * Write a description of class RuneOre here.
 * 
 * @author (your name) 
 * @version (a version number or a date)
 */
public class RuneRock extends Entities
{
    public RuneRock(int x, int y){
        super(x,y);
    }
    ...
    public String closeCollide(){
        Field field=(Field) getWorld();
        String s="";
        if(getX()<296 && getX()>276 && getY()<330 && getY()>285){
            s+="left";
        }

        if(getX()<322 && getX()>=302 && getY()<330 && getY()>285){
            s+="right";
        }

        if(getY()<=305 & getY()>285 && getX()<322 && getX()>276){
            s+="up";
        }

        if(getY()<330 & getY()>315 && getX()<322 && getX()>276){
            s+="down";
        }
        if(!s.equals("")){
            return s;
    ...
    public void paintBoundary(){
        ...
        if(getX()>280 && getX()<320 && getY()>250 && getY()<350){
            if(getY()>300){
                field.paintOrder(RuneRock.class);
            }
            else if(getY()<=300){
